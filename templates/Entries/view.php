...
<section class="entry-view pb-4">
            ...
            <div class="row g-4 align-items-start">
                <div class="col-xl-8">
                    ...
                    <div class="d-flex flex-wrap gap-2 mb-3">
                        <span class="badge entry-badge entry-badge-code"><?= h($reference) ?></span>
                        <span class="badge <?= $entry->active ? 'text-bg-success' : 'text-bg-secondary' ?>">
                            <?= $entry->active ? __('Active') : __('Inactive') ?>
                        </span>
                        <span class="badge text-bg-light border"><?= h($entry->event->event_name) ?></span>
                    </div>
                    <div class="entry-action-row d-flex flex-wrap gap-2">
                        <?= $this->Actions->buttons($entry) ?>
                        <?= $this->Html->link(__('Merge Entry'), ['action' => 'merge', $entry->id], ['class' => 'btn btn-warning']) ?>
                        <?= $this->Html->link(__('Edit Entry'), ['action' => 'edit', $entry->id], ['class' => 'btn btn-outline-dark']) ?>
                    </div>
                </div>
                <div class="col-xl-4">
                    <div class="card border-0 entry-panel h-100">
                        <div class="card-body">
                            ...
                            <div class="entry-contact-line">
                                <div class="entry-contact-label"><?= __('Security Code') ?></div>
                                <div><code><?= h($entry->security_code) ?></code></div>
                            </div>
                            <div class="entry-contact-line mb-0">
                                <div class="entry-contact-label"><?= __('Event') ?></div>
                                <div><?= $this->Html->link($entry->event->event_name, ['controller' => 'Events', 'action' => 'view', $entry->event->id]) ?></div>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div class="col-12 col-xl-6">
            <div class="card shadow-sm h-100">
                ...
                <div class="card-body d-grid gap-2">
                    ...
                    <?= $this->Html->link(__('Open Event'), ['controller' => 'Events', 'action' => 'view', $entry->event->id], ['class' => 'btn btn-outline-secondary text-start']) ?>
                    <?= $this->Html->link(__('Open Merge Tool'), ['action' => 'merge', $entry->id], ['class' => 'btn btn-outline-warning text-start']) ?>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="card shadow-sm mb-4">
        <div class="card-header d-flex justify-content-between align-items-center">
            ...
            <span class="badge text-bg-light border"><?= $this->Number->format($participantCount) ?></span>
        </div>
    ...
    <div class="card shadow-sm">
        <div class="card-header d-flex justify-content-between align-items-center">
            ...
            <span class="badge text-bg-light border"><?= $this->Number->format($checkInCount) ?></span>
        </div>
        <div class="card-body">
                ...
                <div class="entry-checkin-tree">
                    <?php foreach ($entry->check_ins as $checkIn) : ?>
                        <div class="entry-checkin-node">
                            ...
                            <div class="entry-checkin-card-wrap col-12 col-xxl-10">
